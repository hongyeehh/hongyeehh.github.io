---
layout: single
title: "Publications"
permalink: /publications/
author_profile: true
---
{% include base_path %}

You can also find my articles on my [Google Scholar profile](https://scholar.google.com/citations?user=dnaRSnwAAAAJ&hl=en) and latest [CV](/files/cv-yehong.pdf).

## Journal Articles

{% include publication.html
   image="mobilitygen_2026.png"
   title="Deep generative model for human mobility behavior"
   authors="<strong>Ye Hong</strong>, Yatao Zhang, Konrad Schindler, Martin Raubal"
   description="Transportation Research Part C: Emerging Technologies, 2026"
   paper="https://doi.org/10.1016/j.trc.2026.105916"
   code="https://github.com/mie-lab/mobility_generation"
%}

{% include publication.html
   image="clustering_cities_ylbu.jpg"
   title="Bidirectional yet asymmetric causality between urban systems and traffic dynamics in 30 cities worldwide"
   authors="Yatao Zhang, <strong>Ye Hong</strong>, Song Gao, Martin Raubal"
   description="Nature Communications, 2026"
   paper="https://doi.org/10.1038/s41467-026-71377-0"
   code="https://doi.org/10.6084/m9.figshare.28656800"
%}

{% include publication.html
   image="trip_2025.png"
   title="A causal intervention framework for synthesizing mobility data and evaluating predictive neural networks"
   authors="<strong>Ye Hong</strong>, Yanan Xin, Simon Dirmeier, Fernando Perez-Cruz, Martin Raubal"
   description="Transportation Research Interdisciplinary Perspectives, 2025"
   paper="https://doi.org/10.1016/j.trip.2025.101398"
   code="https://github.com/irmlma"
%}

{% include publication.html
   image="lup_25.jpg"
   title="Is a 15-Minute City Within Reach? Measuring Multimodal Accessibility and Carbon Footprint in 12 Major American Cities"
   authors="Tanhua Jin, Kailai Wang, Yanan Xin, Jian Shi, <strong>Ye Hong</strong>, Frank Witlox"
   description="Land Use Policy, 2024"
   paper="https://doi.org/10.1016/j.landusepol.2024.107180"
%}

{% include publication.html
   image="jtrg_2025.jpg"
   title="Evaluating geospatial context information for travel mode detection"
   authors="<strong>Ye Hong</strong>, Emanuel Stüdeli, Martin Raubal"
   description="Journal of Transport Geography, 2023"
   paper="https://doi.org/10.1016/j.jtrangeo.2023.103736"
   code="https://github.com/mie-lab/mode_detect"
%}

{% include publication.html
   image="trc_2023.png"
   title="Context-aware multi-head self-attentional neural network model for next location prediction"
   authors="<strong>Ye Hong</strong>, Yatao Zhang, Konrad Schindler, Martin Raubal"
   description="Transportation Research Part C: Emerging Technologies, 2023"
   paper="https://doi.org/10.1016/j.trc.2023.104315"
   code="https://github.com/mie-lab/location-prediction"
%}

{% include publication.html
   image="ceus_2023.png"
   title="Trackintel: An open-source Python library for human mobility analysis"
   authors="Henry Martin, <strong>Ye Hong</strong>, Nina Wiedemann, Dominik Bucher, Martin Raubal"
   description="Computers, Environment and Urban Systems, 2023"
   paper="https://doi.org/10.1016/j.compenvurbsys.2023.101938"
   code="https://github.com/mie-lab/trackintel"
%}

{% include publication.html
   image="const.png"
   title="Conserved quantities in human mobility: From locations to trips"
   authors="<strong>Ye Hong</strong>, Henry Martin, Yanan Xin, Dominik Bucher, Daniel J Reck, Kay W Axhausen, Martin Raubal"
   description="Transportation Research Part C: Emerging Technologies, 2023"
   paper="https://doi.org/10.1016/j.trc.2022.103979"
%}

{% include publication.html
   image="ijgis_2023.jpg"
   title="Predicting mobile users' next location using the semantically enriched geo-embedding model and the multilayer attention mechanism"
   authors="Yao Yao, Zijin Guo, Chen Dou, Minghui Jia, <strong>Ye Hong</strong>, Qingfeng Guan, Peng Luo"
   description="Computers, Environment and Urban Systems, 2023"
   paper="https://doi.org/10.1016/j.compenvurbsys.2023.102009"
%}

{% include publication.html
   image="lbs_2023.jpg"
   title="Influence of tracking duration on the privacy of individual mobility graphs"
   authors="Nina Wiedemann, Henry Martin, Esra Suel, <strong>Ye Hong</strong>, Yanan Xin"
   description="Journal of Location Based Services, 2023"
   paper="https://doi.org/10.1080/17489725.2023.2239190"
   code="https://github.com/mie-lab/topology_privacy"
%}

{% include publication.html
   image="ijgis_2019.jpg"
   title="Hierarchical community detection and functional area identification with OSM roads and complex graph theory"
   authors="<strong>Ye Hong</strong>, Yao Yao"
   description="International Journal of Geographical Information Science, 2019"
   paper="https://doi.org/10.1080/13658816.2019.1584806"
%}

{% include publication.html
   title="Estimating China's poverty reduction efficiency by integrating multi-source geospatial data and deep learning techniques"
   authors="Yao Yao, Jianhua Zhou, Zhenyu Sun, Qingfeng Guan, Zijin Guo, Yin Xu, Jinwei Zhang, <strong>Ye Hong</strong>, Yun Cai, Range Wang"
   description="Geo-Spatial Information Science, 2023"
   paper="https://doi.org/10.1080/10095020.2023.2165975"
%}

{% include publication.html
   title="Under the Dome: A 3D Urban Texture Model and Its Relationship with Urban Land Surface Temperature"
   authors="Qingfeng Guan, Yao Yao, Ting Ma, <strong>Ye Hong</strong>, Yihong Bie, Jiqin Lyu"
   description="Annals of the American Association of Geographers, 2022"
   paper="https://doi.org/10.1080/24694452.2021.1972790"
%}

{% include publication.html
   title="Discovering the homogeneous geographic domain of human perceptions from street view images"
   authors="Yao Yao, Jin Wang, <strong>Ye Hong</strong>, Chang Qian, Qingfeng Guan, Xiaoyuan Liang, Liang Dai, Jinwei Zhang"
   description="Landscape and Urban Planning, 2021"
   paper="https://doi.org/10.1016/j.landurbplan.2021.104125"
%}

{% include publication.html
   title="Spatiotemporal distribution of human trafficking in China and predicting the locations of missing persons"
   authors="Yao Yao, Yujie Liu, Qingfeng Guan, <strong>Ye Hong</strong>, Ruihe Wang, Ruoqi Wang, Xiaoyuan Liang"
   description="Computers, Environment and Urban Systems, 2021"
   paper="https://doi.org/10.1016/j.compenvurbsys.2020.101567"
%}

{% include publication.html
   title="The Traj2Vec model to quantify residents' spatial trajectories and estimate the proportions of urban land-use types"
   authors="Jinwei Zhang, Xia Li, Yao Yao, <strong>Ye Hong</strong>, Jianke He, Zhuanghe Jiang, Jianghao Sun"
   description="International Journal of Geographical Information Science, 2021"
   paper="https://doi.org/10.1080/13658816.2020.1726923"
%}

{% include publication.html
   title="Analyzing the Effects of Rainfall on Urban Traffic-Congestion Bottlenecks"
   authors="Yao Yao, Dengwei Wu, <strong>Ye Hong</strong>, Di Chen, Zhengyu Liang, Qingfeng Guan, Liu Xun, Liang Dai"
   description="IEEE Journal of Selected Topics in Applied Earth Observations and Remote Sensing, 2020"
   paper="https://doi.org/10.1109/JSTARS.2020.2966591"
%}

{% include publication.html
   title="Exploring the spatial differentiation of urbanization on two sides of the Hu Huanyong Line – based on nighttime light data and cellular automata"
   authors="Di Chen, Yatao Zhang, Yao Yao, <strong>Ye Hong</strong>, Qingfeng Guan"
   description="Applied Geography, 2019"
   paper="https://doi.org/10.1016/j.apgeog.2019.102081"
%}

{% include publication.html
   title="Fine-scale intra- and intercity commercial store site recommendations via multisource big data"
   authors="Yao Yao, Pinde Liu, <strong>Ye Hong</strong>, Zhengyu Liang, Ruoqi Wang, Qingfeng Guan, Jin Chen"
   description="Transactions in GIS, 2019"
   paper="https://doi.org/10.1111/tgis.12553"
%}

{% include publication.html
   title="Mining transition rules of cellular automata for simulating urban expansion by using the deep learning techniques"
   authors="Jianke He, Xia Li, Yao Yao, <strong>Ye Hong</strong>, Jinwei Zhang"
   description="International Journal of Geographical Information Science, 2018"
   paper="https://doi.org/10.1080/13658816.2018.1480783"
%}

{% include publication.html
   title="Estimating the effects of “community opening” policy on alleviating traffic congestion in large Chinese cities by integrating ant colony optimization and complex network analyses"
   authors="Yao Yao, <strong>Ye Hong</strong>, Dengwei Wu, Yatao Zhang, Qingfeng Guan"
   description="Computers, Environment and Urban Systems, 2018"
   paper="https://doi.org/10.1016/j.compenvurbsys.2018.03.005"
%}

{% include publication.html
   title="Mapping fine–scale urban housing prices by fusing remotely sensed imagery and social media data"
   authors="Yao Yao, Jinwei Zhang, <strong>Ye Hong</strong>, Haohuan Liang, Jianke He"
   description="Transactions in GIS, 2018"
   paper="https://doi.org/10.1111/tgis.12330"
%}

{% include publication.html
   title="Classifying urban land use by integrating remote sensing and social medias data"
   authors="Xiaohe Liu, Jianke He, Yao Yao, Jinwei Zhang, Haohuan Liang, Hualiang Wang, <strong>Ye Hong</strong>"
   description="International Journal of Geographical Information Science, 2017"
   paper="https://doi.org/10.1080/13658816.2017.1324976"
%}

{% include publication.html
   title="Simulating urban land-use changes at a large scale by integrating dynamic land parcel subdivision and vector-based cellular automata"
   authors="Yao Yao, Xiaohe Liu, Xia Li, Pinde Liu, <strong>Ye Hong</strong>, Yatao Zhang, Keli Mai"
   description="International Journal of Geographical Information Science, 2017"
   paper="https://doi.org/10.1080/13658816.2017.1360494"
%}

## Conference Proceedings

{% include publication.html
   image="urbanfusion_2026.png"
   title="UrbanFusion: Stochastic Multimodal Fusion for Contrastive Learning of Robust Spatial Representations"
   authors="D. J. Mühlematter, L. Che, <strong>Ye Hong</strong>, Martin Raubal, Nina Wiedemann"
   description="43rd International Conference on Machine Learning (ICML), 2026"
   paper="https://doi.org/10.48550/arXiv.2510.13774"
   code="https://github.com/DominikM198/UrbanFusion"
%}

{% include publication.html
   image="giscience_2023.png"
   title="Predicting visit frequencies to new places"
   authors="Nina Wiedemann, <strong>Ye Hong</strong>, Martin Raubal"
   description="12th International Conference on Geographic Information Science (GIScience), 2023"
   paper="https://doi.org/10.4230/LIPIcs.GIScience.2023.84"
   code="https://github.com/mie-lab/predict-visits"
%}

{% include publication.html
   image="sigspatial_2022.png"
   title="How do you go where? Improving next location prediction by learning travel mode information using transformers"
   authors="<strong>Ye Hong</strong>, Henry Martin, Martin Raubal"
   description="30th ACM SIGSPATIAL, 2022"
   paper="https://doi.org/10.1145/3557915.3560996"
   code="https://github.com/mie-lab/location-mode-prediction"
%}

{% include publication.html
   image="giscience_2021.png"
   title="A clustering-based framework for individual travel behaviour change detection"
   authors="<strong>Ye Hong</strong>, Yanan Xin, Henry Martin, Dominik Bucher, Martin Raubal"
   description="11th International Conference on Geographic Information Science (GIScience), 2021"
   paper="https://doi.org/10.4230/LIPIcs.GIScience.2021.II.4"
   code="https://github.com/mie-lab/change-detection"
%}

{% include publication.html
   title="Causal inference for interpretable and robust deep learning in mobility analysis"
   authors="<strong>Ye Hong</strong>, Martin Raubal"
   description="25th Swiss Transport Research Conference (STRC), 2025"
   paper="https://doi.org/10.3929/ethz-b-000744423"
%}

{% include publication.html
   title="Towards realistic individual activity location demand synthesis using deep generative networks"
   authors="<strong>Ye Hong</strong>, Yatao Zhang, Martin Raubal"
   description="24th Swiss Transport Research Conference (STRC), 2024"
   paper="https://doi.org/10.3929/ethz-b-000683550"
%}

{% include publication.html
   title="Influence of tracking duration on the privacy of individual mobility graphs"
   authors="Henry Martin, Nina Wiedemann, Esra Suel, <strong>Ye Hong</strong>, Yanan Xin"
   description="17th International Conference on Location-Based Services (LBS), 2022"
   paper="https://doi.org/10.3929/ethz-b-000572753"
%}

{% include publication.html
   title="Graph-resnets for short-term traffic forecasts in almost unknown cities"
   authors="Henry Martin, Dominik Bucher, <strong>Ye Hong</strong>, Rene Buffat, Christian Rupprecht, Martin Raubal"
   description="NeurIPS 2019 Competition and Demonstration Track"
   paper="https://doi.org/10.3929/ethz-b-000437682"
%}

## Preprints

{% include publication.html
   title="Interpreting Deep Learning Models for Traffic Forecast: A Case Study of UNet"
   authors="Jin Li, Yanan Xin, <strong>Ye Hong</strong>, Martin Raubal"
   description="Under Review, 2024"
   paper="https://doi.org/10.2139/ssrn.4370154"
%}

{% include publication.html
   title="Synthetic location trajectory generation using categorical diffusion models"
   authors="Simon Dirmeier, <strong>Ye Hong</strong>, Fernando Perez-Cruz"
   description="Under Review, 2024"
   paper="https://doi.org/10.48550/arXiv.2402.12242"
%}

{% include publication.html
   title="Uncertainty Quantification for Image-based Traffic Prediction across Cities"
   authors="Aram Timans, Nina Wiedemann, N. Kumar, <strong>Ye Hong</strong>, Martin Raubal"
   description="Under Review, 2023"
   paper="https://doi.org/10.48550/arXiv.2308.06129"
%}

{% include publication.html
   title="Uncertainty quantification and out-of-distribution detection using surjective normalizing flows"
   authors="Simon Dirmeier, <strong>Ye Hong</strong>, Yanan Xin, Fernando Perez-Cruz"
   description="Under Review, 2023"
   paper="https://doi.org/10.48550/arXiv.2311.00377"
%}
